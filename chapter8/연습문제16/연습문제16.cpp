...
void warp(Mat image)                           // 원근 변환 수행 함수
{
   Point2f dst_pts[4] = {                        // 목적 영상 4개 좌표
      Point2f(0, 0), Point2f(image.cols, 0),
      Point2f(image.cols, image.rows), Point2f(0, image.rows)
   };
   Mat dst;
   Mat perspect_mat = getPerspectiveTransform(pts, dst_pts);      // 원근변환 행렬 계산
   warpPerspective(image, dst, perspect_mat, Size(image.cols, image.rows), INTER_CUBIC);
   imshow("왜곡보정", dst);
}
...
int main()
{
   image = imread("../../image/mine2.jpg", 1);
